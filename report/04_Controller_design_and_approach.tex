...
In order to control turns, when a wall is encountered the \texttt{turnLeft90()}, \texttt{turnRight90()} or \texttt{turn180()} function is called. These three function similarly, as they reset the wheel speed controller to prevent the residual control action from straight driving from affecting the turn, store the current encoder reading for each motor, and set the controller into the corresponding turn mode. Whenever the controller is in turn mode and the \texttt{updateController()} function is called, the turn controller is updated. It computes the absolute encoder count difference between the current and initial values for each wheel and uses the average of both wheels as the measure of turn progress. If the target encoder reading difference needed for a 90 degree or 180 degree turn (depending on the type of turn that is being done) is not reached yet, it sets the motors to turn in opposing directions with a predefined constant speed, in an open-loop actuation (no feedback control). If the target encoder reading has been reached and the exploring is not finished yet, the internal state distance used by the exploration module is reset and the motor is set back into drive mode.
Once the target encoder difference is reached, the motors are briefly stopped and the wheel speed controllers are reset again to ensure a clean transition. A short forward motor command is then applied to reduce mechanical oscillations and stabilize the robot before resuming motion.
If the mouse has finished exploring, it stops.
