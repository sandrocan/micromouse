...
The unregulated 7.4\,V battery voltage is used as the motor-supply domain, so that the H-bridge can provide sufficient drive voltage to the two DC motors. The supplied voltage to the motors is limited to their maximum of 6\,V through PWM. A first regulator, implemented with an \texttt{LM7805}, generates a 5\,V rail. This 5\,V domain supplies the three infrared distance sensors and thereby provides the operating voltage for the analogue wall-detection subsystem. From this 5\,V rail, a second regulator, implemented with an \texttt{LD1117}, generates the 3.3\,V logic supply. The 3.3\,V domain is used by the \texttt{dsPIC33FJ128MC804}, the \texttt{RN4871} Bluetooth module, and the remaining low-voltage digital electronics.
...
After power-up, the firmware performs a defined initialization sequence in which the clock, I/O pins, communication interfaces, timers, PWM hardware, motor and encoder interfaces, ADC with DMA support, and the controller state are configured before exploration begins. After this initialization, the main loop remains empty and the system operates primarily in an interrupt-driven manner.
